...
(
	...
	iPurQuantity int default 0,
	iPurPrice int default 0,
    iVAT int not null,
...
(
	...
	cProID char(6) not null,
	iOrdQuantity int default 0,
	iOrdPrice int default 0,
	iVAT int not null,
...
begin
	...
	if @Table = 'Adm'
        begin
            select @Pre = 'AD'
			select @Num = Max(Right(cAdmID,4)) from Adm
        end
	if @Table = 'Category'
        begin
            select @Pre = 'CA'
...
--Admin
go
if exists (select name from sysobjects where name='prcInsertAdm' and type='p')
drop proc prcInsertAdm 
go
create proc prcInsertAdm
	@vAdmName varchar(50),
...
as
begin
	declare @cAdmID char(6)
	exec prcAutoGeneratedID 'Adm', @cAdmID output
	insert into Employee values(@cAdmID, @vAdmName, @vAdmAddress, @cAdmPhone, @cAdmEmail, @cAdmPassword)
end
...
create proc prcInsertOrders
	@cCusID char(6),
	@nOrdDate nvarchar(50),
	@cStatus char(100),
	@cEmpID char(6)
as
begin
	declare @dDate datetime
	set @dDate = convert(datetime, @nOrdDate)
	declare @cOrdID char(6)
	exec prcAutoGeneratedID 'Orders', @cOrdID output
	insert into Orders(cOrdID,cCusID,dOrdDate,cStatus,cEmpID) values(@cOrdID, @cCusID, @dDate, @cStatus, @cEmpID)
end
...
create proc prcInsertPurchase
	@nPurDate nvarchar(50),
	@cStatus char(100),
	@cEmpID char(6)
as
begin
	declare @dDate datetime
	set @dDate = convert(datetime, @nPurDate)
	declare @cPurID char(6)
	exec prcAutoGeneratedID 'Purchase', @cPurID output
	insert into Purchase(cPurID,dPurDate,cStatus,cEmpID) values(@cPurID, @dDate, @cStatus, @cEmpID)
end
...
begin
	...
	declare @cPurID char(6)
	select @cPurID = cPurID from inserted
	declare @iPurPrice int
	select @iPurPrice = iPurPrice from inserted
	update Purchase set iPurTotalPrice = (iPurTotalPrice + @iPurQuantity*@iPurPrice) where cPurID = @cPurID
end
...
begin
	...
	declare @cOrdID char(6)
	select @cOrdID = cOrdID from inserted
	declare @iOrdPrice int
	select @iOrdPrice = iOrdPrice from inserted
	update Orders set iOrdTotalPrice = (iOrdTotalPrice + @iOrdQuantity*@iOrdPrice) where cOrdID = @cOrdID
...
begin
	...
	declare @cPurID char(6)
	select @cPurID = cPurID from deleted
	declare @iPurPrice int
	select @iPurPrice = iPurPrice from deleted
	update Purchase set iPurTotalPrice = (iPurTotalPrice - @iPurQuantity*@iPurPrice) where cPurID = @cPurID
...
begin
	...
	declare @cOrdID char(6)
	select @cOrdID = cOrdID from deleted
	declare @iOrdPrice int
	select @iOrdPrice = iOrdPrice from deleted
	update Orders set iOrdTotalPrice = (iOrdTotalPrice + @iOrdQuantity*@iOrdPrice) where cOrdID = @cOrdID
...
begin
	...
	declare @iOrdPrice int
	select @iOrdPrice = iOrdPrice from inserted
	update Orders set iOrdTotalPrice = (iOrdTotalPrice - @iOrdQuantityDel*@iOrdPrice + @iOrdQuantityIns*@iOrdPrice) where cOrdID = @cOrdID
...
begin
	...
	declare @iPurPrice int
	select @iPurPrice = iPurPrice from inserted
	update Purchase set iPurTotalPrice = (iPurTotalPrice - @iPurQuantityDel*@iPurPrice + @iPurQuantityIns*@iPurPrice) where cPurID = @cPurID
...
begin
	declare @dDate datetime
	set @dDate = convert(datetime, @nOrdDate)
	update Orders set dOrdDate=@dDate,
	cCusID = @cCusID, 
	cStatus=@cStatus,
	cEmpID=@cEmpID
	where cOrdID=@cOrdID
...
begin
	declare @dDate datetime
	set @dDate = convert(datetime, @nPurDate)
	update Purchase set dPurDate=@dDate,
	cStatus=@cStatus,cEmpID=@cEmpID
	where cPurID=@cPurID
end
--PurDetails
go
...
--Adm
go
create proc prcDeleteAdm(@cAdmID char(6))
as
begin
